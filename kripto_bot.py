"""
Kripto AL-Sinyali Tarayıcı Bot
================================================================
Binance'in (global, ccxt üzerinden) tüm USDT paritelerini tarar.
...
Üçü birden sağlanmadan sinyal ATILMAZ. Bu yüzden bazı turlarda
hiç sinyal gelmeyebilir — bu normaldir, botun "sessiz kalması"
hata değil, kriterlere uyan coin olmadığı anlamına gelir.

SAT sinyali YOKTUR. Bu sadece bir "fırsat tarayıcı"dır, gerçek
para kullanmaz, otomatik işlem açmaz.

Not: Binance TR, ccxt kütüphanesinde ayrı bir borsa olarak
desteklenmiyor. Veri global Binance'ten (aynı piyasa) çekiliyor.
...
Opsiyonel ortam değişkenleri:
    ...
    RSI_MIN            (varsayılan: 50)   -> momentum eşiği
    RSI_MAX            (varsayılan: 75)   -> aşırı alım filtresi (üzerindekiler elenir)
    VOLUME_MULTIPLIER  (varsayılan: 1.5)  -> son hacim, ortalamanın kaç katı olmalı
    CHECK_INTERVAL_SEC (varsayılan: 300)
    EXCLUDE_COINS      (varsayılan: "")   -> örn: "USDC,FDUSD,TUSD"
    COOLDOWN_MIN       (varsayılan: 240)  -> aynı coin için tekrar sinyal atmadan önce kaç dakika beklensin
"""
