...
def analyze(s):
    try:
        ...
        if wick_rejection: score += 5; reasons.append("Dipte satış iğne ile karşılandı")
        ...
        if falling:
            score -= 15; reasons.append("Düşüş trendi devam ediyor")

        distribution = (bp < 58.0 and sr >= 3.0 and m5 < -0.3)
        if distribution:
            score -= 20; reasons.append("Mal yıkma / Dağıtım riski yüksek")
        ...
        if score >= CANDIDATE_THRESHOLD - 5:
            ...
            if old_o is not None and now_o is not None:
                ...
                if oi_change >= 0.8:
                    ...
                    score = clamp(score - 4)
                    reasons.append(f"OI geriliyor (%{oi_change:.2f})")
            DBS.put_oi(s, now_o)
...
def message(r):
    ...
    squeeze_status = f"✅ EVET (%{r['ma_diff_pct']:.2f})" if r["ma_squeeze"] else "❌ HAYIR"
    silent_status = "✅ VAR" if r["silent_accum"] else "❌ YOK"
    
    if r["status"] == "STRONG":
        header = "🐋 BALİNA RADARI — 🟢 GÜÇLÜ ALIM BÖLGESİ"
        warning = "⚠️ Bu tahtada patlama öncesi dip birikimi tamamlandı! Ortalamalar sıkıştı ve akıllı para girişi başladı."
    else:
    ...
    msg = (
        ...
        f"• Ortalamalar Sıkıştı: {squeeze_status}\n"
        f"• Sessiz Birikim: {silent_status}\n"
...
Thread(target=loop, daemon=True, name="balina-v15").start()
