...
class Config:
    ...
    base_gate: int = field(default_factory=lambda: int(os.getenv("BASE_GATE", "42")))
...
def _fetch_derivatives(symbol: str) -> DerivativeMetrics:
    ...
    return DerivativeMetrics(oi_change=oi_change, oi_available=oi_available, funding=funding(symbol))
...
def _score_derivatives(m: BaseMetrics, d: DerivativeMetrics) -> Tuple[int, List[str], str]:
    ...
    oc = d.oi_change
    if d.oi_available:
        if oc >= 8: add(14, f"🐋 OI güçlü artıyor (+%{oc:.1f})")
        elif oc >= 4: add(9, f"📈 OI artıyor (+%{oc:.1f})")

        if abs(m.momentum_15m) <= 1.5 and oc >= 3:
            add(8, "🔮 Fiyat sakin / OI yükseliyor")
            structure = "PRICE_FLAT_OI_UP"
        elif m.momentum_15m > 0 and oc > 0:
            add(5, "📈 Fiyat + OI birlikte yükseliyor")
            structure = "PRICE_UP_OI_UP"
        elif m.momentum_15m > 0 and oc < 0:
            structure = "PRICE_UP_OI_DOWN"

    if d.funding is not None:
        if d.funding < -0.0005: add(6, "⚡ Negatif funding / squeeze potansiyeli")
        elif d.funding <= 0.0005: add(4, "⚖️ Funding dengeli")
        elif d.funding > 0.001: score -= 4; reasons.append("⚠️ Funding fazla pozitif")

    score = max(0, min(CFG.derivative_max_bonus, score))
...
def analyze(symbol: str) -> Dict[str, Any]:
    try:
        candidate = Candidate(symbol=symbol, change_24h=0.0, spot_volume=0.0, futures_volume=0.0)
        metrics, error = extract_base_metrics(candidate)
        if error is not None:
            return error

        if metrics.change_24h >= CFG.extension_24h_pct or metrics.local_position >= CFG.local_high_position:
            if metrics.change_24h >= 25:
                return {"status": "overextended", "score": 0}

        base_score, base_reasons = _score_base(metrics)

        if base_score < CFG.base_gate:
            return {"status": "below_gate", "score": clamp(base_score)}
        ...
        if (metrics.change_24h >= CFG.extension_24h_pct or metrics.local_position >= CFG.local_high_position) and total_score > CFG.overextended_score_cap:
