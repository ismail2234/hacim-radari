...
def telegram_message(r):
    ...
    return (
        ...
        "☁️ ICHIMOKU\n"
        f"Trend: "
        f"{'YÜKSELİŞ ✅' if r.get('ichimoku_bullish', r.get('bullish', False)) else 'ZAYIF'}\n"
        f"Bulut: "
        f"{'ÜSTÜNDE ✅' if r.get('above_cloud', False) else 'ALTINDA ❌'}\n\n"

        "📐 FIB\n"
        f"0.5: "
        f"{r.get('fib_0_5', r.get('fib50', 0)):.8g}\n"
        f"0.618: "
        f"{r.get('fib_0_618', r.get('fib618', 0)):.8g}\n"
        f"0.786: "
        f"{r.get('fib_0_786', r.get('fib786', 0)):.8g}\n"
        f"Bölge: "
        f"{'✅' if r.get('fib_zone', False) else '⏳'}\n\n"

        "📊 VOLUME PROFILE\n"
        f"POC: "
        f"{r.get('poc', 0):.8g}\n"
        f"VA: "
        f"{r.get('va_low', r.get('value_low', 0)):.8g}"
        f" - "
        f"{r.get('va_high', r.get('value_high', 0)):.8g}\n"
        f"Fib + POC: "
        f"{'🔥 KESİŞİM' if r.get('fib_poc', False) else '⏳'}\n\n"

        f"9️⃣ TD: {td_text}\n\n"

        f"📊 Hacim: "
        f"{r.get('volume_ratio', r.get('vr', 0)):.2f}x\n"

        f"📈 RSI: "
        f"{r.get('rsi', r.get('rv', 0)):.1f}\n"

        f"📈 MACD: "
        f"{'✅' if r.get('macd', False) else '❌'}\n"

        f"📈 ADX: "
        f"{r.get('adx', r.get('ad', 0)):.1f}\n"

        f"💠 VWAP: "
        f"{'✅' if r.get('price_above_vwap', False) else '❌'}\n\n"

        f"🛑 Stop: "
        f"{r.get('stop_loss', r.get('stop', 0)):.8g}\n"
