...
def analyze(cfg, client, dbs, market, item):
    ...
    volume_early = 1.10 <= vr <= 3.00
    volume_strong = 1.40 <= vr <= 2.80

    # Kivrim motorunun ilk hacmi
    first_volume = kvol or volume_early
    ...
    add(pre, 18, "PRE-KIVRIM")
    add(turn, 15, "EMA7 KIVRIM")
    add(accel, 7, "KIVRIM İVMESİ")
    add(higher, 12, "HIGHER-LOW")
    add(krsi, 9, "RSI DÖNÜŞÜ")
    add(kmacd, 9, "MACD DÖNÜŞÜ")
    add(kvol, 10, "İLK HACİM")
    add(volume_early, 7, "HACİM ERKEN")
    add(reclaim, 7, "EMA7 GERİ ALIM")

    if early_gate:
        score += 9
        criteria.append("ERKEN KIVRIM V29")

    if pre_signal:
        score += 5
        criteria.append("ERKEN ADAY")
