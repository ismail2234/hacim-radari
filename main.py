Geliyor. Bu kez V15'in çalışan altyapısını bozmayıp, üzerine gerçekten istediğimiz V16 Breakout Predictor mantığını koydum.
En önemli değişiklik: Telegram artık sadece yüksek puan aldığı için coin göndermiyor. Önce yapısal şartları geçmesi gerekiyor. Böylece “çok mesaj” sorununu azaltıyoruz.
import os
import time
import sqlite3
import logging
...
# ============================================================
# 🐋 BALİNA RADARI V16 — BREAKOUT PREDICTOR
# ============================================================
#
# AMAÇ:
# Henüz yükselişini yapmamış,
# dip/birikim bölgesinde bulunan,
# para girişi başlayan,
# sıkışan ve yukarı kırılıma hazırlanan
# az sayıdaki en güçlü coinleri bulmak.
#
# SİNYAL KATMANLARI:
#
# 🔵 PREPARE
#    Dip + birikim + para girişi + sıkışma
#
# 🟢 STRONG BUY
#    Hazırlık + fiyat yapısı dönüşü + kırılım hazırlığı
#
# 🚀 VERY STRONG BUY
#    Çok güçlü dip yapısı + para girişi +
#    breakout'a çok yakın + güçlü teyit
#
# PASS:
#    Telegram'a gönderilmez.
#
# ÖNEMLİ:
# Skor tek başına sinyal üretmez.
# Önce yapısal şartlar sağlanır.
# ============================================================
...
def build_session():
    ...
    session.headers.update({
        "User-Agent": "BalinaRadari-V16/1.0"
    })
...
def candidates(spot, futures):
    ...
    for item in spot:
        ...
        try:
            ...
            # Son 24 saatte zaten patlamış olanları ele.
...
def analyze(symbol):
    ...
    try:
        ...
        # Çok sert düşen bıçağı yakalamaya çalışma.
        if (
            live_change < -2.0
            or m5 < -3.5
        ):
        ...
        # Fiyat son dakikalarda yeni dip yapmayı bırakıyor mu?
        base_holding = (
            price >= lowest_recent
            and lows[-1] >= lowest_recent
        )

        # Son 3 mumun dipleri birbirine yaklaşıyor mu?
        low_range = max(last_lows[-3:]) - min(last_lows[-3:])

        base_width_pct = (
            low_range / price * 100.0
            if price > 0
            else 99.0
        )
        ...
        # MA7 artık yukarı kıvrılıyor mu?
        ...
        near_breakout = (
            0.0 <= breakout_distance <= 0.80
        )

        very_near_breakout = (
            0.0 <= breakout_distance <= 0.40
        )

        # Fiyat direnci test ediyor mu?
        resistance_test = (
        ...
        spot_ratio = (
            average(spot_volumes[-3:])
            / avg_spot
        )

        futures_ratio = (
            average(futures_volumes[-3:])
            / avg_futures
        )
        ...
        # OI sadece güçlü adaylarda çekiliyor.
        preliminary_strength = (
            low_zone
            and buyer_pressure >= 62
            and (
                spot_ratio >= 2
                or silent_accumulation
            )
        )
...
def message(r):
    ...
    if r["status"] == "VERY_STRONG":
        ...
        footer = (
            "🔥 Dip yapısı + para girişi + "
            "kırılım hazırlığı çok güçlü."
        )
        ...
        footer = (
            "🎯 Dip yapısı tamamlanıyor ve "
            "yukarı kırılım teyidi güçleniyor."
        )
        ...
        footer = (
            "👁️ Erken aşama. Dip/birikim yapısı "
            "oluşuyor, kırılım henüz tamamlanmadı."
        )
    ...
    return (
        ...
        f"• Dip tabanı: "
        f"{'✅ VAR' if r['base_formed'] else '❌ YOK'}\n\n"
        ...
        f"• Spot öncü: "
        f"{'✅' if r['spot_leading'] else '❌'}\n\n"

        "📐 KIRILIM HAZIRLIĞI\n"
        f"• MA sıkışması: "
        f"{'✅' if r['ma_squeeze'] else '❌'}"
        f" (%{r['ma_diff']:.2f})\n"

        f"• Sessiz birikim: "
        f"{'✅' if r['silent_accumulation'] else '❌'}\n"

        f"• Volatilite daralması: "
        f"{'✅' if r['volatility_compression'] else '❌'}\n"

        f"• Kırılıma mesafe: "
        f"%{r['breakout_distance']:.2f}\n"

        f"• Hacim ivmesi: "
        f"{r['volume_acceleration']:.2f}x\n\n"
        ...
        f"• Tepe kırılımı: "
        f"{'✅' if r['break_high'] else '❌'}\n"

        f"• Satış reddi: "
        f"{'✅' if r['wick_rejection'] else '❌'}\n\n"
...
def scan():
    ...
    log.info(
        "🐋 V16 | Aday:%d | 🚀ÇOK GÜÇLÜ:%d | 🟢GÜÇLÜ:%d | 🔵HAZIRLIK:%d | Geç:%d | Zayıf:%d | Hata:%d | Gönder:%d | Süre:%.1fs",
...
V16'da özellikle değiştirdiğim kritik şey
Artık bir coin sırf 90 puan aldı diye 🚀 Çok Çok Güçlü Al olmuyor.
Örneğin VERY_STRONG için aynı anda:
dip/birikim + para akışı + yüksek alıcı baskısı + price action dönüşü + kırılıma yakınlık + erken fiyat + güçlü spot akışı
aranıyor.
Ayrıca kırılıma mesafeyi Telegram'a açıkça yazdırıyorum. Böylece mesajı gördüğünde sadece 94/100 değil, “bu coin neden şimdi dikkatimi çekmeli?” sorusunun cevabını göreceksin.
Bir önemli not: Bu sistem “geleceği kesin bilir” şeklinde çalışamaz; kırılım öncesi olasılık yapısını arar. Dolayısıyla 🚀 etiketi bile garanti değil, sadece V16'nın en sıkı filtresinden geçen sinyal olacak.
