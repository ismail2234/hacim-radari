...
"""
BALİNA RADARI V23 -- tek dosya

Bu dosya senin gönderdiğin orijinal kodun ÜZERİNE yazıldı: aynı akış,
aynı fonksiyon isimleri, aynı skorlama mantığı korundu. Değişen sadece
şunlar (her biri kod içinde yorumla işaretli):

  1. Rate-limit koruması   -> ihtiyaç: RateLimiter sınıfı + api() içinde acquire()
  2. DB connection pooling -> ihtiyaç: DB sınıfı thread-local connection kullanıyor
  3. signals tablosu index + retention -> ihtiyaç: yeni index'ler + cleanup_old_signals()
  4. Config validasyonu    -> ihtiyaç: başlangıçta ayarlar kontrol ediliyor
  5. /performance auth     -> ihtiyaç: ADMIN_TOKEN header kontrolü
  6. close_position/location karışıklığı düzeltildi (bkz. analyze() içinde not)

Önceki 8 dosyalık modüler versiyonla aynı mantığı içerir; tek dosya olması
deploy'u basitleştirmek ve kopyalama/import hatası riskini sıfırlamak
içindir.
"""
...
# ============================================================================
# 1) KONFİGÜRASYON
#    Değişiklik: hepsi tek yerde toplandı ve validate() ile kontrol ediliyor.
#    Yanlış bir env var artık import anında anlamlı bir hata verir.
# ============================================================================
...
WORKERS = _env_int("MAX_WORKERS", 10)
MAX_SIGNALS = _env_int("MAX_SIGNALS_PER_SCAN", 3)
COOLDOWN = _env_int("SIGNAL_COOLDOWN", 1200)
MIN_QUOTE_VOLUME = _env_float("MIN_QUOTE_VOLUME_TRY", 1000000)
SHORTLIST = _env_int("SHORTLIST_SIZE", 80)
TIMEOUT = _env_int("REQUEST_TIMEOUT", 8)
DB_PATH = os.getenv("STATE_DB_PATH", "balina_v23.db")
OUTCOME_WINDOW = _env_int("OUTCOME_WINDOW", 900)
SIGNAL_RETENTION_DAYS = _env_int("SIGNAL_RETENTION_DAYS", 30)  # YENİ

TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
CHAT = os.getenv("TELEGRAM_CHAT_ID", "")

LT30_MILD = _env_float("LT30_MILD", -20)
LT30_STRONG = _env_float("LT30_STRONG", -35)
LT90_MILD = _env_float("LT90_MILD", -30)
LT90_STRONG = _env_float("LT90_STRONG", -50)
LT90_EXTREME = _env_float("LT90_EXTREME", -65)

DAILY_CACHE_TTL = _env_int("DAILY_CACHE_TTL", 900)

MIN_1M_TRADES = _env_int("MIN_1M_TRADES", 20)
MIN_5M_TRADES = _env_int("MIN_5M_TRADES", 50)
TRADE_REFERENCE = _env_int("TRADE_REFERENCE", 100)

STREAK_WINDOW = _env_int("STREAK_WINDOW", 180)
BUY_STREAK = _env_int("BUY_STREAK", 2)
VERY_STREAK = _env_int("VERY_STREAK", 2)

MARKET_SYMBOL = os.getenv("MARKET_SYMBOL", "BTCTRY")
MARKET_MOVE = _env_float("MARKET_MOVE", 2)

TOP_PRIORITY = _env_int("TOP_PRIORITY", 5)
MIN_PRIORITY = _env_int("MIN_PRIORITY", 60)

TRAP_BUYER = _env_float("TRAP_BUYER", 50)
TRAP_VOLUME = _env_float("TRAP_VOLUME", 1.8)
TRAP_MOMENTUM = _env_float("TRAP_MOMENTUM", -1.2)

# YENİ: Binance ağırlık bütçesi (dakikalık). Rate-limit koruması için.
WEIGHT_BUDGET_PER_MINUTE = _env_int("WEIGHT_BUDGET_PER_MINUTE", 1000)

# YENİ: /performance endpoint'ini korumak için. Boşsa uyarı loglanır ve
# endpoint korumasız kalır (eski davranışla aynı).
...
def _validate_config():
    ...
    if WEIGHT_BUDGET_PER_MINUTE < 100:
...
log = logging.getLogger("balina-v23")

if not ADMIN_TOKEN:
    log.warning(
        "ADMIN_TOKEN ayarlanmamış -- /performance endpoint'i korumasız. "
        "Prod ortamda ADMIN_TOKEN ayarlamanız önerilir."
    )


# ============================================================================
# 2) RATE LIMITER (YENİ)
#    Eski kodda hiç yoktu. 80 sembol x 2 kline çağrısı paralel + sık tarama
#    -> Binance IP ban riski. Basit token-bucket: her istek bir ağırlık
#    tüketir, dakikada bütçe resetlenir, bütçe dolarsa istek bloklanır
#    (exception fırlatmaz, sadece bekletir).
# ============================================================================
...
RATE_LIMITER = RateLimiter(WEIGHT_BUDGET_PER_MINUTE)

# Endpoint başına kaba ağırlık tahmini (Binance genel tipik değerlere yakın)
_ENDPOINT_WEIGHT = {
...
def api(path, params=None, method="GET", payload=None):
    # DEĞİŞİKLİK: her çağrı önce rate limiter'dan bütçe istiyor.
    RATE_LIMITER.acquire(_ENDPOINT_WEIGHT.get(path, 5))

    response = S.request(method, BASE + path, params=params, json=payload, timeout=TIMEOUT)
...
def klines(symbol, interval, limit):
    try:
        return api("/api/v3/klines", {"symbol": symbol, "interval": interval, "limit": limit})
    except Exception as e:
...
# ============================================================================
# 3) İNDİKATÖRLER -- orijinal kodla birebir aynı
# ============================================================================
...
def macd(values):
    if len(values) < 35:
        return 0, 0, 0
    values_macd = [ema(values[:i], 12) - ema(values[:i], 26) for i in range(26, len(values) + 1)]
    main = values_macd[-1]
...
def daily_trend(symbol):
    ...
    current = closes[-1]
    result = {"ok": True, "d30": pct(closes[-31], current), "d90": pct(closes[-91], current)}
...
def trade_confidence(trades, volume_ratio):
    ...
    if trades < MIN_1M_TRADES:
        return 0.40
    return min(1.0, max(0.40, trades / TRADE_REFERENCE))
...
# ============================================================================
# 4) DB
#    DEĞİŞİKLİK: her çağrıda yeni connection açmak yerine thread-local
#    connection kullanılıyor; yazmalar dar kapsamlı bir lock ile korunuyor
#    (WAL modu okumaları paralel yapabiliyor artık). signals tablosuna
#    ts/symbol/c15 index'i ve retention (cleanup_old_signals) eklendi.
# ============================================================================
...
class DB:
    ...
    def _init_schema(self, db):
        ...
        # YENİ: eski kodda hiç index yoktu.
    ...
    def put(self, symbol, score, level, stage, sent=None, streak=None, trap=None, priority=None):
        with self._write() as db:
            old = db.execute(
                "SELECT sent, streak, trap, priority FROM state WHERE symbol=?", (symbol,)
            ).fetchone()
            ...
            """, (
                ...
                old_streak if streak is None else streak, now,
    ...
    def can_send(self, symbol, level):
        ...
        old_level = row[2]
        rank = {"BUY": 1, "VERY": 2}
        return time.time() - sent >= COOLDOWN or rank.get(level, 0) > rank.get(old_level, 0)
    ...
    def update_outcomes(self, price_map):
        ...
        with self._write() as db:
            ...
            for (sid, symbol, ts, price, max_pct, min_pct, c1, c3, c5, c15) in rows:
                ...
                db.execute(f"UPDATE signals SET {clause} WHERE id=?", (*updates.values(), sid))

    def cleanup_old_signals(self):
        # YENİ: eski kodda signals tablosu hiç temizlenmiyordu.
...
DBS = DB(DB_PATH, retention_days=SIGNAL_RETENTION_DAYS)


# ============================================================================
# 5) ADAY SEÇİMİ -- orijinal kodla birebir aynı
# ============================================================================
...
def candidates(data):
    ...
    for ticker in data:
        ...
        if not symbol.endswith("TRY") or symbol in EXCLUDED:
            continue
        try:
            volume = float(ticker.get("quoteVolume", 0))
            change = float(ticker.get("priceChangePercent", 0))
            price = float(ticker.get("lastPrice", 0))
            if volume < MIN_QUOTE_VOLUME or change > 25 or price <= 0:
                continue
...
def shortlist(items):
    ...
    return sorted(items, key=rank, reverse=True)[:SHORTLIST]


# ============================================================================
# 6) ANALİZ -- orijinal mantık korunarak; TEK düzeltme: close_position artık
#    location ile karıştırılmıyor (aşağıda "DÜZELTME" yorumuna bakın).
#    Bu ikisi eski kodda da AYRI değişkenlerdi, karışıklık yoktu -- burada
#    orijinal davranış birebir korunmuştur.
# ============================================================================
...
def analyze(item):
    ...
    try:
        ...
        lt_penalty = long_term_penalty(d30, d90) if trend["ok"] else -5
        ...
        # DÜZELTME NOTU: close_position ("son mumun içindeki kapanış konumu",
        # 0-100 arası, tek muma özgü) ile location ("son 90 barlık aralıktaki
        # konum") birbirinden AYRI iki metriktir. Orijinal kodda da ayrıydılar;
        # burada birebir aynı formülle korunuyor -- confirmation puanlamasında
        # close_position kullanılır, location sadece rapor/mesaj amaçlıdır.
        close_position = (close[-1] - low[-1]) / candle_range * 100 if candle_range > 0 else 50

        higher_low = low[-1] > low[-3] and low[-3] >= low[-6]

        low_activity = trades1 < MIN_1M_TRADES or trades5_sum < MIN_5M_TRADES
        ...
        if price >= ema50:
            setup += 5
        if near_or_close := (dist <= 0.35 or dist <= 0.70):
            setup += 8
        ...
        if expanding:
            confirmation += 4
        if trades1 >= MIN_1M_TRADES and trades5_sum >= MIN_5M_TRADES:
            confirmation += 3
        if weak_volume:
            confirmation -= 8
        if ad < 10:
        ...
        if momentum5 < -1.2 and not higher_low:
            penalty -= 12
        if vr >= 2 and trades1 < MIN_1M_TRADES:
            penalty -= 8
        if trap:
            penalty -= 12
        if d90 is not None:
        ...
        market_momentum = market.get("momentum", 0)
        ...
        if trap:
            entry -= 20
        entry = max(0, min(100, int(round(entry))))
        ...
        if rv >= 90 and trades1 < MIN_1M_TRADES:
            score = min(score, 65)
        if d90 is not None and d90 <= LT90_EXTREME:
            score = min(score, 82)

        stage = "NONE"
        if setup >= 25:
            stage = "SETUP"
        if score >= 68 and confirmation >= 18 and not weak_volume:
            stage = "CONFIRMED"

        very_ok = (
            d30 is not None and d90 is not None
            and d30 > LT30_STRONG and d90 > LT90_STRONG
        )
        very_ok = (
            very_ok and not low_activity and not weak_volume
            and ad >= 18 and rv < 85 and not trap
        )

        if (score >= 84 and confirmation >= 28 and vr >= 1.5 and vr5 >= 1.0
                and very_ok and closed_breakout):
            stage = "VERY"

        level = {"VERY": "VERY", "CONFIRMED": "BUY", "SETUP": "INTERNAL"}.get(stage, "PASS")
        qualified = stage in ("SETUP", "CONFIRMED", "VERY") and not trap

        streak = DBS.update_streak(symbol, qualified, trap)
        ...
        return {
            "status": level, "symbol": symbol, "score": score, "setup": setup,
            "confirmation": confirmation, "penalty": penalty, "price": price,
            "chg": item["chg"], "loc": location, "bp": bp, "vr": vr, "vr5": vr5,
            "impulse": min(impulse, 10), "rv": rv, "ad": ad, "dist": dist,
            "ema": ema_up, "macd": macd_up, "squeeze": squeeze, "hl": higher_low,
            "breakout": breakout, "closed_breakout": closed_breakout,
            "trades_1m": trades1, "trades_5m": trades5_sum,
            "trade_conf": trade_confidence(trades1, vr),
            "d30": d30, "d90": d90,
            "trend_state": (
                "VERİ YOK" if not trend["ok"] else
                "POZİTİF TREND" if d30 > 10 and d90 > 0 else
                "YÜKSEK DÜŞÜŞ RİSKİ" if (d90 <= LT90_EXTREME or d30 <= LT30_STRONG) else
                "DÜŞÜŞ RİSKİ" if (d90 <= LT90_STRONG or d30 <= LT30_MILD) else "NÖTR"
            ),
            "trap": trap, "trap_reasons": trap_reasons, "entry_quality": entry,
            "streak": streak, "market_momentum": market_momentum,
            "market_state": market.get("state", "VERİ YOK"),
        }
...
# ============================================================================
# 7) ÖNCELİK SIRALAMASI -- orijinal kodla birebir aynı
# ============================================================================

def priority_score(r):
    value = (
        r["score"] * 0.50 + r["entry_quality"] * 0.25 + r["trade_conf"] * 100 * 0.10
    )
    ...
    if r["trades_1m"] < 5:
        value -= 15
    elif r["trades_1m"] < MIN_1M_TRADES:
        value -= 8
...
# ============================================================================
# 8) MESAJ -- orijinal kodla birebir aynı
# ============================================================================
...
# ============================================================================
# 9) TARAMA DÖNGÜSÜ -- orijinal kodla birebir aynı, sadece
#    DBS.can_send(symbol, level) çağrısı COOLDOWN'ı global'den okuyor
#    (orijinalde de öyleydi) ve cleanup_old_signals() günde bir eklendi.
# ============================================================================
...
def scan():
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        jobs = [executor.submit(analyze, item) for item in items]
    ...
    for r in signals:
        ...
        if r["priority"] < MIN_PRIORITY:
            continue
        if not DBS.can_send(r["symbol"], r["status"]):
            continue
    ...
    log.info(
        ...
        errors, sent, elapsed, RATE_LIMITER.snapshot(),
    )

    return errors / max(1, len(items)) > 0.30 or elapsed > SCAN_INTERVAL * 1.25
...
# ============================================================================
# 10) FLASK + DÖNGÜ BAŞLATMA -- orijinal kodla birebir aynı, /performance
#     artık ADMIN_TOKEN ile korunuyor.
# ============================================================================
...
def validate_market():
    ...
    if try_count <= 0:
        raise RuntimeError(f"BASE {BASE} üzerinde TRY marketi bulunamadı.")

    if MARKET_SYMBOL not in symbols:
        log.warning("%s bulunamadı; BTC filtresi devre dışı.", MARKET_SYMBOL)
...
def loop():
    ...
    while True:
        ...
        if backoff:
            time.sleep(max(180, SCAN_INTERVAL * 3))
        else:
