...
@app.route("/")
def home():
    return "🐋 Balina Radarı V2.1 Aktif!"
...
def get_tickers():
    ...
    try:
        ...
        if not isinstance(data, list):
            logger.error(
                "Binance beklenmeyen cevap döndürdü: %s",
                data
            )
            return []
...
def analyze_coin(symbol):
    ...
    try:
        ...
        # Kapanmamış son mumu kullanmıyoruz.
        candles = candles[:-1]
        ...
        if score >= 90:
            signal_type = "🚨 ÇOK GÜÇLÜ BALİNA SİNYALİ"

        elif score >= 80:
            signal_type = "🔥 GÜÇLÜ BALİNA SİNYALİ"

        else:
            signal_type = "🐋 BALİNA AKÜMÜLASYON SİNYALİ"
...
def scanner():
    ...
    logger.info(
        "🐋 Balina Radarı V2.1 başladı."
    )

    # --------------------------------------------------------
    # BAŞLANGIÇ TELEGRAM TESTİ
    # --------------------------------------------------------

    startup_message = (
        "🐋 BALİNA RADARI V2.1 AKTİF\n\n"
        "✅ Bot başlatıldı.\n"
        "🔎 Binance taraması hazırlanıyor.\n"
    ...
    if telegram_ok:
        logger.info(
            "✅ Başlangıç Telegram mesajı gönderildi."
        )
    else:
        logger.error(
            "❌ Başlangıç Telegram mesajı gönderilemedi."
        )
    ...
    while True:
        ...
        try:
            ...
            if not tickers:
                ...
                logger.error(
                    "❌ Binance'ten ticker verisi alınamadı."
                )

            candidates = []

            # 24 saatlik hacim filtresi
            for coin in tickers:
        ...
        logger.info(
            "✅ Tarama tamamlandı. "
            "%.0f saniye sonra tekrar taranacak.",
            wait_time
        )
...
if __name__ == "__main__":
    ...
    if (
        ...
        or TELEGRAM_BOT_TOKEN.startswith("BURAYA_")
    ):
        raise RuntimeError(
    ...
    if (
        ...
        or TELEGRAM_CHAT_ID.startswith("BURAYA_")
    ):
        raise RuntimeError(
