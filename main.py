...
# ============================================================
# BALINA RADARI V16.1 — SIGNAL PROGRESSION EDITION
# ============================================================
...
def build_session():
    ...
    s.headers.update({"User-Agent": "BalinaRadari-V16.1/1.0"})
    return s
...
class DB:
    def __init__(self, path):
        ...
        with self.lock, sqlite3.connect(path) as d:
            ...
            d.execute(
                ...
                "symbol TEXT PRIMARY KEY,stage INTEGER,"
                "stage_ts REAL,stage_score REAL)"
            )
    ...
    def get_oi(self, symbol):
        ...
        if not row or time.time() - row[1] > SCAN_INTERVAL * 5:
            return None
        return float(row[0])
    ...
    def set_stage(self, symbol, stage, score):
        with self.lock, sqlite3.connect(self.path) as d:
            d.execute(
                "INSERT INTO progression(symbol,stage,stage_ts,stage_score) "
                "VALUES(?,?,?,?) "
                "ON CONFLICT(symbol) DO UPDATE SET "
                "stage=excluded.stage,stage_ts=excluded.stage_ts,"
                "stage_score=excluded.stage_score",
                (symbol, stage, time.time(), score)
            )
...
def candidates(spot, futures):
    ...
    for x in spot:
        ...
        if (
            ...
            or any(
                symbol.endswith(z)
                for z in ("UPUSDT", "DOWNUSDT", "BULLUSDT", "BEARUSDT")
            )
        ):
            continue
        ...
        try:
            ...
            if spot_vol < MIN_VOLUME or fut_vol < MIN_VOLUME:
                continue
...
def analyze(symbol):
    try:
        sp = klines(SPOT, symbol, "1m", 48)
        fu = klines(FUT, symbol, "1m", 36)
        sp5 = klines(SPOT, symbol, "5m", 18)
        ...
        closes = [float(x[4]) for x in sp]
        ma7 = average(closes[-7:])
        ma30 = average(closes[-30:])
        ma_diff_pct = abs(ma7 - ma30) / price * 100.0
        ma_squeeze = ma_diff_pct <= 0.85
        ma_rising = ma7 > average(closes[-10:-3])
        ...
        last3_lows = [float(x[3]) for x in closed[-3:]]
        base_low = min(last3_lows)
        base_high = max(last3_lows)
        base_spread = (base_high - base_low) / price * 100.0
        base_forming = base_spread <= 0.45 and price >= base_low * 0.998
        ...
        higher_low = a_low > b_low and float(live[3]) >= a_low
        ...
        wick_rejection = (
            lower_wick > 0
            and lower_wick >= max(body * 0.8, price * 0.0003)
        )
        ...
        prev_vol = average(sv[-6:-3])
        vol_acc = average(sv[-3:]) / prev_vol if prev_vol > 0 else 1.0

        silent_accum = (
            -0.35 <= lc <= 0.45
            and (sr >= 2.2 or fr >= 2.2)
        )
        ...
        spot_leads = sr >= 1.8 and sr >= fr * 1.05

        recent_ranges = [
            (float(x[2]) - float(x[3])) / float(x[4]) * 100.0
            for x in sp[-6:] if float(x[4]) > 0
        ]
        old_ranges = [
            (float(x[2]) - float(x[3])) / float(x[4]) * 100.0
            for x in sp[-18:-6] if float(x[4]) > 0
        ]

        recent_range = average(recent_ranges)
        old_range = average(old_ranges)
        volatility_compression = (
            old_range > 0 and recent_range <= old_range * 0.75
        )

        resistance = max(float(x[2]) for x in closed[-5:])
        breakout_distance = (
            max(0.0, (resistance - price) / price * 100.0)
            if price > 0 else 99.0
        )
        close_to_breakout = 0.0 <= breakout_distance <= 0.60
        ...
        if very_low:
            score += 20
            reasons.append("🟦 Cok guclu yerel dip bolgesi")
        elif near_low:
            score += 14
            reasons.append("🟦 Yerel dip/birikim bolgesi")

        if base_forming:
            score += 8
            reasons.append("🧱 Dip tabani olusuyor")

        if ma_squeeze:
            score += 15
            reasons.append(f"📐 MA sikismasi (%{ma_diff_pct:.2f})")

        if ma_rising:
            score += 5
            reasons.append("📈 Kisa MA yukari donuyor")
        ...
        if volatility_compression:
            score += 6
            reasons.append("🤏 Volatilite daraliyor")

        if higher_low:
            score += 7
            reasons.append("📐 Higher-Low olustu")
        if break_high:
            score += 8
            reasons.append("💥 Onceki tepe kirildi")
        if wick_rejection:
            score += 5
            reasons.append("🛡️ Dipte satis reddedildi")

        if sr >= 3.5:
            score += 12
            reasons.append(f"🐋 Spot akisi cok guclu ({sr:.2f}x)")
        elif sr >= 2.0:
            score += 8
            reasons.append(f"📈 Spot akisi basladi ({sr:.2f}x)")

        if fr >= 1.8:
            score += 7
            reasons.append(f"⚡ Futures destekliyor ({fr:.2f}x)")
        if trr >= 1.8:
            score += 7
            reasons.append(f"🤖 Islem sayisi artti ({trr:.2f}x)")

        if bp >= 80:
            score += 12
            reasons.append(f"🐋 Cok guclu alici baskisi (%{bp:.1f})")
        elif bp >= 70:
            score += 9
            reasons.append(f"🟢 Guclu alici baskisi (%{bp:.1f})")
        elif bp >= 62:
            score += 5
            reasons.append(f"🟢 Pozitif alici akisi (%{bp:.1f})")

        if spot_leads:
            score += 5
            reasons.append("🐋 Spot akisi onculuk ediyor")

        if close_to_breakout:
            score += 6
            reasons.append(
                f"🎯 Kirilima cok yakin (%{breakout_distance:.2f})"
            )

        if vol_acc >= 2.0:
            score += 6
            reasons.append(f"🚀 Hacim ivmesi guclu ({vol_acc:.2f}x)")
        elif vol_acc >= 1.3:
            score += 4
            reasons.append(f"🔥 Hacim ivmesi artiyor ({vol_acc:.2f}x)")

        if falling:
            score -= 15
            reasons.append("⚠️ Dusus yapisi devam ediyor")
        if distribution:
            score -= 20
            reasons.append("⚠️ Dagitim riski")
        ...
        preparation = (
            ...
            and (base_forming or ma_squeeze or silent_accum)
            and bp >= 60
            and not falling
        ...
        strong = (
            ...
            and bp >= 68
            and (higher_low or wick_rejection or ma_rising)
            and (spot_leads or sr >= 2.5 or silent_accum)
        )
        ...
        rocket = (
            ...
            and (
                break_high
                or (close_to_breakout and reversal and ma_rising)
            )
            and m5 <= 1.80
            and lc <= 1.00
        )
        ...
        if rocket:
            status, stage, signal_type = "ROCKET", 3, "🚀 COK COK GUCLU AL"
        elif strong:
            status, stage, signal_type = "STRONG", 2, "🟢 GUCLU AL"
        elif preparation:
            status, stage, signal_type = "PREP", 1, "🔵 HAZIRLIK AL"
        else:
            status, stage, signal_type = "PASS", 0, "⚪ PASS"
        ...
        log.exception("%s analiz hatasi: %s", symbol, e)
        return {"status": "error"}


def message(r, previous_stage=0, elapsed_minutes=None):
    if r["status"] == "ROCKET":
        header = "🐋 BALINA RADARI V16.1 🚀 COK COK GUCLU AL"
        footer = (
            "🚀 Ana tetik olustu. Dip + para akisi + donus + "
            "kirilim kosullari birlikte guclendi."
        )
    elif r["status"] == "STRONG":
        header = "🐋 BALINA RADARI V16.1 🟢 GUCLU AL"
        footer = (
            "🟢 Hazirlik yapisi guclendi. Donus teyitleri "
            "belirginlesiyor."
        )
    else:
        header = "🐋 BALINA RADARI V16.1 🔵 HAZIRLIK AL"
        footer = (
            "👁️ Erken asama. Dip/birikim yapisi var; "
            "sonraki teyit bekleniyor."
        )

    transition = ""
    if previous_stage > 0 and r["stage"] > previous_stage:
        labels = {
            1: "🔵 HAZIRLIK",
            2: "🟢 GUCLU AL",
            3: "🚀 COK COK GUCLU AL"
        ...
        transition = (
            f"📈 ASAMA YUKSELDI: "
            f"{labels.get(previous_stage, 'ONCEKI')} → "
            f"{labels.get(r['stage'], 'YENI')}
"
        )
        if elapsed_minutes is not None:
            transition += (
                f"⏱️ Onceki asamadan sonra: "
...
"
    ...
    squeeze = (
        f"✅ %{r['ma_diff_pct']:.2f}"
        if r["ma_squeeze"] else "❌ HAYIR"
    )
    silent = "✅ VAR" if r["silent_accum"] else "❌ YOK"
    base = "✅ VAR" if r["base_forming"] else "❌ YOK"
    volcomp = "✅ VAR" if r["volatility_compression"] else "❌ YOK"
    ...
    return (
        ...
        f"• Islem sayisi: {r['trr']:.2f}x\n"
        f"• Alici baskisi: %{r['bp']:.1f}\n"
        f"• Spot oncu: {'✅' if r['spot_leads'] else '❌'}\n\n"
        "📐 KIRILIM HAZIRLIGI\n"
        f"• MA sikismasi: {squeeze}\n"
        f"• Sessiz birikim: {silent}\n"
        f"• Volatilite daralmasi: {volcomp}\n"
        f"• Kirilima mesafe: %{r['breakout_distance']:.2f}\n"
        f"• Hacim ivmesi: {r['vol_acc']:.2f}x\n\n"
        "📈 PRICE ACTION\n"
        f"• Higher-Low: {'✅' if r['higher_low'] else '❌'}\n"
        f"• Tepe kirilimi: {'✅' if r['break_high'] else '❌'}\n"
        f"• Satis reddi: {'✅' if r['wick_rejection'] else '❌'}\n\n"
        "⚡ MOMENTUM\n"
        f"• Canli 1m: {r['lc']:+.2f}%\n"
        f"• 5m: {r['m5']:+.2f}%\n"
        f"• 15m: {r['m15']:+.2f}%\n"
        ...
        f"{reasons}\n\n"
        f"{footer}\n"
        "⚠️ Teknik filtredir; risk yonetimi sana aittir."
    )
