...
def telegram_message(r):
    ...
    return (
        "🐋 BALİNA RADARI V28 KIVRIM\n\n"
        f"{title} | {confirm}\n"
        f"🪙 #{r.get('symbol', '?')}\n"
        f"💰 {r.get('price', 0):.8g}\n"
        f"🎯 Skor: {r.get('score', 0):.0f}/100\n"
        f"🌀 Kıvrım: {stage}\n"
        f"🎯 Kıvrım Skoru: {r.get('kivrim_score', 0):.0f}/100\n"
        f"⚡ Kıvrım Erkenlik: {r.get('kivrim_early_score', 0):.0f}/100\n"
        f"↪️ EMA7 Kıvrım: {'✅' if r.get('kivrim_turning', False) else '❌'}\n"
        f"📉 Higher-Low: {'✅' if r.get('kivrim_higher_low', False) else '❌'}\n\n"
        "☁️ ICHIMOKU\n"
        f"Trend: {'YÜKSELİŞ ✅' if r.get('ichimoku_bullish', r.get('bullish', False)) else 'ZAYIF'}\n"
        f"Bulut: {'ÜSTÜNDE ✅' if r.get('above_cloud', False) else 'ALTINDA ❌'}\n\n"
        "📐 FIB\n"
        f"0.5: {r.get('fib_0_5', r.get('fib50', 0)):.8g}\n"
        f"0.618: {r.get('fib_0_618', r.get('fib618', 0)):.8g}\n"
        f"0.786: {r.get('fib_0_786', r.get('fib786', 0)):.8g}\n"
        f"Bölge: {'✅' if r.get('fib_zone', False) else '⏳'}\n\n"
        "📊 VOLUME PROFILE\n"
        f"POC: {r.get('poc', 0):.8g}\n"
        f"VA: {r.get('va_low', r.get('value_low', 0)):.8g}"
        f" - {r.get('va_high', r.get('value_high', 0)):.8g}\n"
        f"Fib + POC: {'🔥 KESİŞİM' if r.get('fib_poc', False) else '⏳'}\n\n"
        f"9️⃣ TD: {td_text}\n\n"
        f"📊 Hacim: {r.get('volume_ratio', r.get('vr', 0)):.2f}x\n"
        f"📈 RSI: {r.get('rsi', r.get('rv', 0)):.1f}\n"
        f"📈 MACD: {'✅' if r.get('macd', False) else '❌'}\n"
        f"📈 ADX: {r.get('adx', r.get('ad', 0)):.1f}\n"
        f"💠 VWAP: {'✅' if r.get('price_above_vwap', False) else '❌'}\n\n"
        f"🛑 Stop: {r.get('stop_loss', r.get('stop', 0)):.8g}\n"
        f"📉 Risk: %{r.get('stop_distance', 0):.2f}\n\n"
        f"🎯 Teyitler: {criteria_text}\n"
        f"🌀 Kıvrım Detay: {r.get('kivrim_reasons_text', '')}\n\n"
...
def scan():
    ...
    try:
        ...
        log.info("V28 tarama başladı | aday:%d", len(items))
        ...
        try:
            ...
            try:
                ...
                log.exception("rank_signals hatası")
                ranked = signals
        except Exception:
            log.exception("rank_signals hatası")
            ranked = signals
        ...
        for result in ranked:
            ...
            try:
                ...
                log.exception("Cooldown hatası | %s", symbol)
                can_send = False
            ...
            try:
                ...
                log.exception("Telegram hatası | %s", symbol)
                message_id = False
            ...
            try:
                ...
                log.exception("DB sinyal kaydı | %s", symbol)
            ...
            try:
                ...
                log.exception("DB state kaydı | %s", symbol)

            log.info(
                "V28 SİNYAL | %s | %s | skor=%s | kivrim=%s | kivrim_early=%s",
                symbol,
                level,
                result.get("score", 0),
        ...
        try:
            ...
            log.exception("Outcome hatası")

        elapsed = time.time() - started

        log.info(
            "V28 | aday:%d | sonuç:%d | sinyal:%d | gönder:%d | %.1fs",
            len(items),
            len(results),
            len(signals),
            sent,
        ...
        log.exception("SCAN genel hatası")
        return True


def loop():
    log.info("🐋 BALİNA RADARI V28 KIVRIM BAŞLADI")
    ...
    try:
        ...
        log.exception("Binance kontrol hatası")

    if SETTINGS.telegram_token and SETTINGS.telegram_chat:
        try:
            CLIENT.telegram(
                "🐋 BALİNA RADARI V28 KIVRIM AKTİF\n"
                "Kıvrım + EMA7 + Dip + Higher-Low + "
                "RSI + MACD + Hacim + Ichimoku + "
                "Fibonacci + Volume Profile"
            )
        except Exception:
            log.exception("Telegram aktivasyon hatası")
    ...
    while True:
        ...
        if time.time() - last_cleanup > 86400:
            try:
                ...
                log.exception("DB temizlik hatası")
...
@app.route("/")
def home():
    return "🐋 Balina Radarı V28 Kıvrım Aktif"
