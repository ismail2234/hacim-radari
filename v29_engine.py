...
def _add_v29_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    data["vwap"] = (
        (typical * vol).cumsum() /
        cumulative_vol.replace(0, pd.NA)
    ).fillna(data["close"])
...
def _selling_rejection(data: pd.DataFrame) -> bool:
    ...
    lower_wick = _f(min(last["open"], last["close"]) - last["low"])
    close_position = _f((last["close"] - last["low"]) / candle_range)

    return lower_wick / candle_range >= 0.35 and close_position >= 0.55


def _absorption_score(data: pd.DataFrame, volume_ratio: float) -> int:
    if volume_ratio < 1.3:
        return 0
...
def _earlyness_score(
    ...
    if open_price > 0:
        candle_change = (close - open_price) / open_price * 100
...
def _fake_risk(data: pd.DataFrame, volume_ratio: float) -> tuple[int, list[str]]:
    last = data.iloc[-1]
    prev = data.iloc[-2]
    risk = 0
    reasons = []
...
def calculate_v29_signal(df: pd.DataFrame) -> dict[str, Any]:
    if len(df) < 60:
        return {
            "version": "V29",
            "signal": "WAIT",
    ...
    volume_ratio = _f(volume["volume_ratio"])
    volume_acceleration = detect_volume_acceleration(data)

    v28 = calculate_v28_signal(df)

    curve_score, curve_reasons = _curve_score(
        data, volume_ratio, volume_acceleration
    )
    earlyness_score = _earlyness_score(data, volume_ratio)
    movement_score, movement_reasons = _movement_score(
        data, volume_ratio, volume_acceleration
    )
    absorption_score = _absorption_score(data, volume_ratio)
    fake_risk, fake_reasons = _fake_risk(data, volume_ratio)

    # V29 ağırlıkları: kıvrım ve erkenlik ana hedef.
    score = _clamp(
        curve_score * 0.45
    ...
    return {
        ...
        "reasons": list(dict.fromkeys(curve_reasons + movement_reasons)),
    }
