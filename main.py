...
def message(r: dict) -> str:
    # YENİ (Faz B): üç aşamalı başlık -- ÖNCÜ AL artık gerçekten
    # gönderilen, ayrı bir tier (eskiden SETUP hiç Telegram'a gitmiyordu).
    title = {
        "ONCU": "🟡 ÖNCÜ AL",
        "BUY": "🟢 AL",
        "VERY": "🚀 GÜÇLÜ AL",
...
def scan() -> bool:
    ...
    for r in signals:
        ...
        min_p = SETTINGS.min_priority_oncu if r["status"] == "ONCU" else SETTINGS.min_priority
        if r["priority"] < min_p:
            continue
    ...
    log.info(
        "V24 | TRY:%d/%d | ÖNCÜ:%d | AL:%d | VERY:%d | Hata:%d | Gönder:%d | %.1fs | budget:%s",
        len(items), len(all_candidates), stats.get("ONCU", 0), stats.get("BUY", 0),
