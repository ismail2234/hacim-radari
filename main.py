...
def scan():
    ...
    for item in data:
        try:
            price_map[item.get("symbol")] = float(
...
def performance():
    ...
    if not rows:
        return {
            "samples": 0,
            "note": "Henüz tamamlanmış sinyal yok."
        }
    ...
    def stats(data):
        ...
        return {
            ...
            "completed": len(done),
            "avg_15m_pct": round(
                avg([r[6] for r in done]),
                2
            ),
            "positive_15m_pct": round(
                sum(r[6] > 0 for r in done)
                / len(done) * 100,
                1
            )
        }
...
@app.route("/")
def home():
    return "🐋 Balina Radarı V22.1 Aktif"
...
def validate_market():
    ...
    log.info(
        "V22.1 | Binance TR doğrulandı | TRY:%d",
        try_count
    )
...
def loop():
    log.info(
        "🐋 BALİNA RADARI V22.1 başlatılıyor..."
    )
